...
func (c *conn) readProtocolLine() ([]byte, error) {
...
func (c *conn) readProtocolReply() ([]byte, error) {
	line, err := c.readProtocolLine()
	if err != nil {
		return nil, err
	}
	if len(line) == 0 {
...
func (c *conn) DoProtocol(p []byte) ([]byte,error){
	//c.mu.Lock()
	//pending := c.pending
	//c.pending = 0
	//c.mu.Unlock()
	if len(p) == 0 {
		return nil, errors.New("send protocol empty")
	}
